...
async def setup_learner():
    ...
    try:
        # Newer PyTorch versions default to weights_only=True which will fail
        # when the checkpoint is a full pickled object. Explicitly request
        # full load (weights_only=False). If the installed torch doesn't
        # support the arg, fall back to allowlisting the ResNet class
        # (trusted local checkpoint) and retry loading.
        try:
            learn = torch.load(path / export_file_name, map_location=torch.device('cpu'), weights_only=False)
        except TypeError:
            # Older/newer torch may not accept weights_only kwarg or may
            # require safe globals to unpickle certain classes.
            try:
                import torchvision
                torch.serialization.add_safe_globals([torchvision.models.resnet.ResNet])
            except Exception:
                # If add_safe_globals isn't available, continue and try a plain load
                pass
...
def load_model(path=".", model_name="cattle_breed_classifier_full_model.pth"):
    # Mirror the same safe-loading logic used in setup_learner
    try:
        try:
        ...
        # Re-raise so callers can handle/log the error as before
        raise
    return learn
